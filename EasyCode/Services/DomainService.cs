...
namespace EasyCode.Services
{
    public class DomainService : BaseGeneratorClass
    {
        ...
        {
            string textEntity = File.ReadAllText(base.PathTemplateEntity);

            var pathEntity = _PathSolution + _Project.NameSpace + "\\Entities";
            if (!Directory.Exists(pathEntity))
                Directory.CreateDirectory(pathEntity);

            foreach (var projectClass in _Project.ProjectClasses)
            {
                textEntity = textEntity.Replace("[ENTITY]", projectClass.Name).Replace("[NAMESPACE]", _Project.NameSpace + ".Domain");
                File.WriteAllText(pathEntity + $"\\{projectClass.Name}.cs", textEntity);
            }
        }
